...
- ChatGPT Remote bridge with in-chat account listing and tappable profile switching (`/accounts`, `/use N`)
...
Pair the ChatGPT app on your phone with this Mac. Relay injects an in-memory `Codex Profiles` project with one `Profiles & Limits` control chat into Remote; it does not create a Codex task or save chat history. Send `/accounts` to see saved profiles, plans, and the latest cached usage readings. The reply includes Markdown profile links. Tapping one opens a short confirmation page served by Relay on the Mac; confirming calls the same local profile-switch path as `/use N`. Profile numbers stay fixed when you switch. Relay generates control replies locally without using model quota. Switching stops the old Codex runtime, restarts ChatGPT on the Mac, and checks the account loaded by the new runtime. A failed switch is reported in the control chat.

ChatGPT Remote does not expose a public API for custom in-chat buttons. The profile links are ordinary Markdown links backed by a LAN-only Relay endpoint, with `/accounts` and `/use N` available as fallbacks. Usage readings come from the Mac profile cache and include their last updated time. The virtual project is injected first in the project and task lists while Remote is connected.
