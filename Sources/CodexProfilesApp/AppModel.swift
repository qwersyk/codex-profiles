...
final class AppModel: ObservableObject {
    ...
    private func codexCLIURL() throws -> URL {
        let app = try codexAppURL()
        let candidates = [
            app.appendingPathComponent("Contents/Resources/codex-cli/bin/codex"),
        ...
        if let bundledCLI = candidates.first(where: { FileManager.default.isExecutableFile(atPath: $0.path) }) {
            return bundledCLI
        }
