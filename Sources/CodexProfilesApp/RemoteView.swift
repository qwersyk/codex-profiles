...
struct RemoteView: View {
        ...
        HStack(alignment: .top, spacing: 24) {
            ...
            Button { remote.toggleConnection() } label: {
                Image(systemName: remote.enabled ? "link" : "link.badge.plus")
                    ...
                    .foregroundStyle(remote.connected && remote.desktopReady ? Color.green : remote.enabled ? .orange : .secondary)
            ...
            VStack(spacing: 12) {
                Button { Task { await model.openRemoteDesktop() } } label: { device("laptopcomputer", attention: remote.needsDesktopAttention && !model.isWorking) }
                    .buttonStyle(.plain)
                    .help(remote.needsDesktopAttention ? "Open or reconnect ChatGPT to enable Remote" : "Open ChatGPT")
                    .accessibilityLabel(remote.needsDesktopAttention ? "Reconnect ChatGPT — action required" : "Open ChatGPT")
                Menu {
                    ForEach(rows) { row in
    ...
    @ViewBuilder private func device(_ symbol: String, attention: Bool = false) -> some View {
        let icon = Image(systemName: symbol).font(.system(size: 35, weight: .light))
            .foregroundStyle(attention ? Color.orange : .primary)
        let content = Group {
            if #available(macOS 14, *) {
                icon.symbolEffect(.pulse, options: .repeating, isActive: attention && !reduceMotion)
            } else { icon }
        }.frame(width: 110, height: 80)
        Group {
            if #available(macOS 26, *) {
                content.glassEffect(.regular.tint(attention ? .orange.opacity(0.12) : nil).interactive(), in: RoundedRectangle(cornerRadius: 20))
            } else { content.background(.ultraThinMaterial, in: RoundedRectangle(cornerRadius: 20)) }
        }
        .overlay(RoundedRectangle(cornerRadius: 20).strokeBorder(Color.orange.opacity(attention ? 0.65 : 0), lineWidth: 1.5))
    }
}
