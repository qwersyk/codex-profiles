...
func ensureRuntime(_ paths: RelayPaths, args: [String]) throws {
    ...
    // Profile switching replaces CODEX_HOME/auth.json. Keep the detached
    // app-server on that file instead of an older credential in Keychain.
    p.arguments = ["-c", "cli_auth_credentials_store=\"file\""] + forwarded
        + ["--listen", "unix://" + paths.socket.path]
